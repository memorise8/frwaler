...
- **완주**: 크롤러가 `_mark_exhausted()`로 "목록 끝까지 걸었다"고 명시했을
  ...
  보고해도 동일하게 완주로 처리합니다(단, 정상 반환과 동시에 취소가 도착한
  경우는 예외 — 아래 「취소」 참고). 완주해도 `cursor` 자체는 지우지 않습니다.
  뒤늦게 재개된 `backfill` 작업이 그 커서를 다시 전진시키면 `completed_at`은
  자동으로 지워집니다(그 사이트가 아직 끝나지 않았다는 뜻이므로). 반면
  oldest_first 증분(`mode=incremental`)이 이어서 커서를 전진시키는 것은 완주한
  사이트에서도 일어나는 정상적인 루틴 동작이라 `completed_at`을 지우지
  않습니다 — 그렇지 않으면 화면이 "아직 안 끝났다"고 거꾸로 거짓말하게 됩니다.

  **주의:** 완주 판정은 크롤러가 목록의 자연스러운 끝을 스스로 구분해 보고하는
  데 의존합니다. 일부 사이트(mof-go-kr-doc, repositorio-uchile-cl-discover,
  etera-ee-browse, datacatalogue-adruk-org-browser 등)는 200 응답이지만 예상과
  다른 마크업/모양을 돌려주는 경우를 "목록 끝"과 구분하지 못하고 같은 값으로
  처리할 수 있습니다(파싱 레벨의 빈 결과). 어떤 사이트가 유독 이르게 완주로
  표시된다 싶으면, 사이트의 현재 페이지 구조를 확인한 뒤 `POST /jobs`로 백필을
  다시 시작해 보세요.

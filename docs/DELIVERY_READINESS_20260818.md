...
### 5. A1 — 실패를 성공으로 보고하는 문제 · **납품 후로 미뤄도 됨** (A2는 완료)
...
A1 은 워커가 "예외 없이 끝남"을 "성공"으로 취급하는 데서 온다 — 크롤러 반환 계약까지
손대야 해서 크다. A2 는 경과 시간으로 판정해 계약을 건드리지 않고 닫았다. A1 은
크롤러 원문 로그로 진단은 가능하다 — 목록에서 한눈에 안 보일 뿐이다.

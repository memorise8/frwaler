...
뒤에 한 번에 고치는 편이 낫다. → 아래 2026-08-18 후속 참조(문서 교정 완료).

**2026-08-18 후속:** 전수 대조 완료 — `scripts/audit/capacity_corrected.csv`.
보정 총계 **24,330,001건**(위 표의 20,480,139 는 5개 사이트만 반영한 중간값).
2배 이상 상향 370개 사이트, 값 없는(no_data) 사이트 0개(단, 24곳은 측정값이 문자 그대로 0 — 정보 없음이 아니라 0으로 기록된 값이다). single_source 로 표시된
사이트는 실측 재검증 후보다. 추정 저장 용량은 **약 31 TB(추정)**(30.8 TB,
사이트별 실측 PDF 비율 기반 — 위 문단의 "17.7 TB"와 §3·§6 의 "100 TB 이상"은 모두
이 값으로 대체된다). 방법별 신뢰도와 원인 분석은
...
`[2026-08-18 보정: 약 30.8 TB(추정) — §2 후속 참조]`

...
Generated from `repair_publicsafety-gc-ca-cnt.csv` — 1 previously-unmeasured sites (all `sites` rows minus those with a numeric source_total already in coverage_report.csv/custom_crawler_totals(.SNAPSHOT).csv, minus HAL-global-scope hal_solr sites >1.5M).

## Results

- Targets: **1**
- Completed (exact count, crawl finished before the external cap): **1**
...
- This pass's counted total (sum of `counted`, exact + lower-bound rows): **3**
...
- **Combined capacity estimate: 19,633,505**

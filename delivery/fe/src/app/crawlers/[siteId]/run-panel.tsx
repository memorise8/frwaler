  ...
  const submit = async () => {
    ...
    try {
      const response = await fetch("/api/jobs", {
        ...
        body: JSON.stringify({ siteId, mode, limit: Number(limit) }),
      });
  ...
  return (
    <section className="run-panel" aria-labelledby="run-heading">
      ...
      <div className="run-fields">
        <label><span>수집 모드</span><select value={mode} onChange={(event) => setMode(event.target.value === "full" ? "full" : "incremental")}><option value="incremental">증분 수집</option><option value="full">전체 수집</option></select></label>
        <label><span>최대 저장 건수</span><input min="1" onChange={(event) => setLimit(event.target.value)} type="number" value={limit} /></label>
        <button className="run-submit" disabled={state.kind === "submitting"} onClick={submit} type="button">{state.kind === "submitting" ? "등록 중…" : "크롤러 실행"}</button>
      </div>

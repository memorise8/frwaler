...
export default async function Home({ searchParams }: Readonly<{ searchParams: Params }>) {
  ...
  return (
    <div className="status-page">
      ...
      <section className="database-status" aria-label="실제 데이터베이스 현황">
        ...
        {databaseStats ? <>
          <div className="summary-grid">
            ...
            <article className="summary-card"><span>PDF 확보</span><strong>{databaseStats.overview.pdf_downloaded.toLocaleString("ko-KR")}</strong><small>{(databaseStats.overview.pdf_bytes / 1024 ** 4).toFixed(1)} TB 메타데이터 합계</small></article>
            <article className="summary-card"><span>텍스트 확보</span><strong>{databaseStats.overview.text_extracted.toLocaleString("ko-KR")}</strong><small>추출 완료 문서</small></article>
            <article className="summary-card"><span>최신 수집</span><strong className="summary-date">{databaseStats.overview.latest_collected_at ? new Date(databaseStats.overview.latest_collected_at).toLocaleDateString("ko-KR") : "없음"}</strong><small>DB collected_at 기준</small></article>
          </div>
      ...
      <section className="taxonomy-grid" aria-label="국가 및 자료 유형 분류">
        <article className="taxonomy-panel">

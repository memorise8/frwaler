...
export default function ScheduleManager({initialRows,sites}:{initialRows:Schedule[];sites:SiteOption[]}) {
  ...
  const save=async(siteId:string,body:object)=>{setMessage(null);const response=await fetch(`/api/schedules/${encodeURIComponent(siteId)}`,{method:"PUT",headers:{"content-type":"application/json"},body:JSON.stringify(body)});if(!response.ok){setMessage({text:"예약을 저장하지 못했습니다.",error:true});return;}const row=await response.json() as Schedule;setRows(old=>[...old.filter(item=>item.site_id!==row.site_id),row].sort((a,b)=>a.site_id.localeCompare(b.site_id)));setMessage({text:`${siteId} 예약을 저장했습니다.`,error:false});};

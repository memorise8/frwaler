...
[[ -n "${DELIVERY_OBSERVATION_URL:-}" && -n "${DELIVERY_API_TOKEN:-}" ]] || { echo "observation URL/token required" >&2;exit 2; }
...
python3 - "$GPU_FREE_MIB" "$GPU_UTIL" "$DISK_FREE" "$HEALTH" > /tmp/libertree-llm-metrics.json <<'PY'
import json,sys
print(json.dumps({"worker_id":"gpu0-observer","gpu_memory_free_bytes":int(sys.argv[1])*1024*1024,
 ...
 "endpoint_healthy":sys.argv[4]=="true"},separators=(",",":")))
PY

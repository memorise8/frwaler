# -*- coding: utf-8 -*-
"""Crawler for Democritus University of Thrace DSpace repository (repo.lib.duth.gr/jspui).

Strategy:
  - simple-search is blocked by the site's Apache WAF (403 on the whole path,
    regardless of query string), so listing goes through browse?type=title
    with sort_by=3 (submit date) & order=DESC — this returns a flat,
    non-grouped item listing (unlike type=dateissued, which is grouped by
    distinct date value on this instance) paginated via offset=N.
  - Detail page: handle/{handle}?mode=full — same DSpace HEAL metadata
    profile as other DSpace 6.3 JSPUI sites in this codebase:
...
_SITE_ID = "repo-lib-duth-gr-jspui"
_BASE = "https://repo.lib.duth.gr"
_LIST_TPL = (
    _BASE + "/jspui/browse"
    "?type=title&sort_by=3&order=DESC&rpp=20&etal=-1&offset={offset}"
)
...
def _extract_handles(html: str) -> list[str]:
    """Return deduplicated item handle strings from a browse-listing page."""
...
def _build_paper(handle: str, parsed: dict) -> dict | None:
    ...
    # --- Department (graduate program / sub-unit) ---
    department = (
        _first(fields.get("dc.contributor.master", []))
        or _first(fields.get("heal.academicPublisher", []))
    )

    # --- Journal ---
    journal = _first(fields.get("heal.journalName", []))
    ...
    for uri in fields.get("dc.identifier.uri", []):
        if "doi.org" in uri:
            ...
            break
    if not doi:
        doi = _first(fields.get("dc.identifier.doi", []))
...
class RepoLibDuthGrJspuiCrawler(BaseCrawler):
    """Crawler for Democritus University of Thrace DSpace repository (all item types)."""

    site_id = "repo-lib-duth-gr-jspui"
    site_name = "Custom: repo-lib-duth-gr-jspui"
    base_url = "https://repo.lib.duth.gr"
    ...
    def crawl(self, limit=None) -> int:
        ...
        while True:
            ...
            # -- Fetch list page --
            list_url = _LIST_TPL.format(offset=offset)
            raw_list = _curl_get(list_url)
            if not raw_list:
                print(f"[{self.site_id}] Failed to fetch list at offset={offset}. Stopping.")
                break

            handles = _extract_handles(raw_list)
            if not handles:
                print(f"[{self.site_id}] No item handles at offset={offset}. End of results.")
                break

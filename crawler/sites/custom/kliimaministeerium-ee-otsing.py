# -*- coding: utf-8 -*-
"""Crawler for Kliimaministeerium (Estonia Climate Ministry) news.

Endpoints:
  list  GET https://search.service.eu-live.vportal.ee/v1/search/kliimamin
             ?sort_by=created&type=Uudis&page={p}
        Requires: Referer + Origin headers from https://kliimaministeerium.ee
        Returns Solr JSON: response.numFound, response.docs[].
        Default page size = 10, page is 0-indexed.

  detail  GET https://kliimaministeerium.ee{doc.uri}
        HTML page; abstract in .node__content; PDF links via a[href$=.pdf].
"""
...
_SITE_ID = "kliimaministeerium-ee-otsing"


class KliimaministeeriumEeOtsingCrawler(BaseCrawler):
    site_id = "kliimaministeerium-ee-otsing"
    site_name = "Custom: kliimaministeerium-ee-otsing"
    base_url = "https://kliimaministeerium.ee"

    _API_URL = "https://search.service.eu-live.vportal.ee/v1/search/kliimamin"
    _LIST_TYPE = "Uudis"
    _SORT_BY = "created"
    ...
    def _curl(self, url, *, timeout=None, extra_headers=None):
        ...
        cmd = [
            ...
            "-H", "Referer: https://kliimaministeerium.ee/",
            "-H", "Origin: https://kliimaministeerium.ee",
        ]
        if extra_headers:
    ...
    def _node_id_from_ss_id(ss_search_api_id):
        """Extract numeric node ID from 'entity:node/6967:et' → '6967'."""
    ...
    def _build_abstract_from_api(self, doc):
        ...
        # Collect body parts from content array, skipping title repetition and
        # short strings (author names, stray punctuation).
        body_parts = []
        ...
        if lead and body_parts:
            # Avoid duplicating lead text if it's already the first body part
    ...
    def crawl(self, limit=None):
        ...
        print(
            f"[{_SITE_ID}] detail: {self.base_url}{{uri}}"
        )
        ...
        for page in range(self._MAX_PAGES):
            ...
            for doc in docs:
                ...
                try:
                    ...
                    # Fetch detail page for rich abstract + PDF links
                    ...
                    # Build abstract: prefer detail page body, fall back to API content
            ...
            # If this page had no new URLs (all seen), we've looped back — stop.
            if not page_had_new:

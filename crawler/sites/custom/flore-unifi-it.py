# -*- coding: utf-8 -*-
"""IRIS — University of Florence repository crawler (OAI-PMH, DIDL).

IRIS is CINECA's Italian DSpace-CRIS variant. Its HTML UI and DSpace REST API
(``/server/api``) both sit behind Cloudflare (probe: 403 "Just a moment..."),
but the OAI-PMH endpoint at ``/oai/request`` is NOT challenged and serves the
full catalogue. So this crawler harvests OAI-PMH instead of scraping HTML.

Metadata prefix ``didl`` (MPEG-21 DIDL) is used because it embeds BOTH the
Dublin-Core block (title/creator/description/date/type/identifier/rights) AND
the ORIGINAL-bundle bitstream URL as ``<Resource mimeType="application/pdf"
ref="https://flore.unifi.it/bitstream/.../file.pdf">`` — so title, abstract and
PDF link all come from a single harvest pass (``oai_dc`` alone carries no file
link). Pagination is via OAI ``resumptionToken`` (100 records/page).

Only records that are (a) ``info:eu-repo/semantics/openAccess`` AND (b) carry a
PDF Resource are saved — mirroring the source sheet's
``descriptionFulltext=open`` filter.

PDF DOWNLOAD CAVEAT: the ``/bitstream/...`` host is behind the same Cloudflare
WAF as the HTML/REST layer, so ``pdf_url`` is recorded but the direct download
returns 403 to plain ``requests``/``curl_cffi`` (verified). PDFs are therefore
left pending for a later browser-based (playwright) download pass; metadata is
captured in full now.
"""
...
class FloreUnifiItCrawler(BaseCrawler):
    site_id = "flore-unifi-it"
    site_name = "Custom: flore-unifi-it"
    base_url = "https://flore.unifi.it"
    PUBLISHER = "University of Florence"

    OAI_ENDPOINT = base_url + "/oai/request"
    METADATA_PREFIX = "didl"
    MAX_PAGES = 400            # 400 * 100 = 40k records safety cap
    ...
    # for robustness (plain curl intermittently fails the TLS handshake on the
    # sibling FLORE instance; chrome131 fingerprint is reliable on both).
    # ------------------------------------------------------------------
    ...
    def _fetch_page(self, resumption_token=None, context="page"):
        ...
        try:
            ...
            return None, None
        # OAI-level error (e.g. noRecordsMatch, badResumptionToken)
    ...
    def _record_to_paper(self, record):
        ...
        # identifiers: handle URL (canonical), DOI, external links

# -*- coding: utf-8 -*-
"""Crawler for KU Leuven RDR Dataverse datasets.

Discovered endpoints:
  - List:   /api/search?q=*&type=dataset&sort=dateSort&order=desc
...
class RdrKuleuvenBeDataverseCrawler(BaseCrawler):
    site_id = "rdr-kuleuven-be-dataverse"
    site_name = "Custom: rdr-kuleuven-be-dataverse"
    ...
    def _departments(self, fields):
        field = fields.get("departmentFaculty") or {}
        value = field.get("value")
        if isinstance(value, list):
            return self._join_values(value)
        if isinstance(value, str):
    ...
    def _persistent_id(self, item):
        ...
        url = item.get("url") or ""
        if "10.48804/" in url:
            return "doi:" + url.split("10.48804/", 1)[1].strip("/")
        return None

# -*- coding: utf-8 -*-
"""HAL Science AFD crawler.

Uses the public HAL Solr API (api.archives-ouvertes.fr/search/AFD/) to collect
REPORT and OTHER document types submitted with attached files from
Agence française de développement (AFD).

Starting URL reference:
  https://hal.science/AFD/search/index/?q=%2A&rows=30&submitType_s=file&docType_s=REPORT+OR+OTHER
"""
...
_API_URL = "https://api.archives-ouvertes.fr/search/AFD/"
_FIELDS = (
    "docid,halId_s,title_s,abstract_s,authFullName_s,"
    "producedDate_s,submittedDate_s,docType_s,"
    "keyword_s,files_s,fileMain_s,doi_s,journalTitle_s,journalIssn_s,"
    "publisher_s,instStructName_s,labStructName_s,domain_s,language_s,label_s"
)
_ROWS = 30
_MAX_PAGES = 200
...
class HalScienceAfdCrawler(BaseCrawler):
    site_id = "hal-science-afd"
    site_name = "Custom: hal-science-afd"
    ...
    def _extract_filename(url: str) -> str:
        """Pull the URL-decoded last path segment as the original filename."""
        if not url:
            return ""
        ...
        for page in range(_MAX_PAGES):
            ...
            for doc in docs:
                ...
                try:
                    ...
                    uri = f"https://hal.science/{hal_id}" if hal_id else ""

                    # URL-based deduplication
                    dedup_key = uri or docid
                    ...
                    journal_issn = str(doc.get("journalIssn_s") or "")
                    ...
                    # PDF URL: prefer direct files_s link, fall back to fileMain_s
                    files_list = doc.get("files_s") or []
                    file_main = str(doc.get("fileMain_s") or "")
                    if files_list:
                        pdf_url = str(files_list[0])

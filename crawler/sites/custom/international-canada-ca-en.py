...
(international.canada.ca domain)."""
...
SITE_ID = "international-canada-ca-en"

INDEX_URL = (
    "https://international.canada.ca/en/global-affairs/corporate/reports/"
    "access-information-privacy-reports-global-affairs"
)
...
class InternationalCanadaCaEnCrawler(BaseCrawler):
    site_id = "international-canada-ca-en"
    site_name = "Custom: international-canada-ca-en"
    base_url = "https://international.canada.ca"
    ...
    def crawl(self, limit=None):
        ...
        print(f"[{SITE_ID}] page {page_num}: saved {saved}/{limit_disp}")
        ...
        if page_num >= MAX_PAGES:
            print(f"[{SITE_ID}] reached safety cap of {MAX_PAGES} pages.")
    ...
    def _parse_list(self, soup: BeautifulSoup) -> list[dict]:
        ...
        list_base = self._canonical_url(soup) or self.base_url
    ...
    def _curl_get(self, url: str, context: str = "request") -> str | None:
        ...
        # The archive.org wayback proxy serves a JS proof-of-work challenge
        # page (Archive-It bot protection) when a browser-like User-Agent
        # is present; omitting it returns the archived content directly.

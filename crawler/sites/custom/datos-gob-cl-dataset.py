# -*- coding: utf-8 -*-
"""datos.gob.cl dataset crawler.

The public dataset page is a React/CKAN shell. The records behind the target
URL are exposed through CKAN's package_search/package_show API endpoints.
"""
...
class DatosGobClDatasetCrawler(BaseCrawler):
    site_id = "datos-gob-cl-dataset"
    site_name = "Custom: datos-gob-cl-dataset"
    base_url = "https://datos.gob.cl"

    START_URL = (
        "https://datos.gob.cl/dataset/"
        "?q=a+OR+res_name%3Aa+OR+res_description%3Aa"
        "&fq=filter%28res_format%3Apdf%29+filter%28license_id%3Acc-nc%29"
    )
    LIST_API_URL = "https://datos.gob.cl/api/3/action/package_search"
    DETAIL_API_URL = "https://datos.gob.cl/api/3/action/package_show"
    DETAIL_PAGE_URL = "https://datos.gob.cl/dataset/{name}"
    SEARCH_QUERY = "a OR res_name:a OR res_description:a"
    FILTER_QUERY = "filter(res_format:pdf) filter(license_id:cc-nc)"
    PAGE_SIZE = 10
    ...
    def crawl(self, limit=None):
        ...
        while True:
            ...
            list_data = self._fetch_list(start)
            if not list_data:
            ...
            for idx, record in enumerate(records, start=start + 1):
                ...
                try:
                    ...
                    detail = self._fetch_detail(package_key)
                    if detail is self._NETWORK_FAILED:
                        raise RuntimeError("detail API failed after retries")
                    elif detail:
                        parsed = self._parse_package(detail, source_format="json")
                    else:
                        detail_html = self._fetch_detail_html(package_key)
                        if not detail_html:
                        ...
                        parsed = self._parse_detail_html(soup, record, package_key)
    ...
    def _parse_package(self, item, source_format):
        ...
        if not pdf_resources:
            raise RuntimeError(f"{package_id} has no PDF resource")

        abstract = self._build_abstract(item, pdf_resources)
        ...
        pdf_resource = pdf_resources[0]

        published_date = (
            self._normalize_date(item.get("metadata_modified"))
            or self._normalize_date(item.get("metadata_created"))
            or self._normalize_date(pdf_resource.get("last_modified"))
            or self._normalize_date(pdf_resource.get("created"))
        )
        ...
        metadata = {
            ...
            "groups": groups,
            "resources": [self._resource_summary(resource) for resource in pdf_resources],
        }
        ...
        return {
            ...
            "url": url,
            "pdf_url": pdf_resource.get("url") or "",
            "doi": "",
    ...
    def _parse_detail_html(self, soup, record, package_key):
        ...
        title = re.sub(r"\s+-\s+Portal de Datos Abiertos\s*$", "", title or "").strip()
    ...
    def _build_abstract(self, item, pdf_resources):
        parts = []
    ...
    def _add_part(self, parts, value):
        ...
        if text and text not in parts and text.lower() not in {"no informado", "sin descripcion", "sin descripción"}:
            parts.append(text)
    ...
    def _pdf_resources(self, resources):
        ...
        for resource in resources:
            ...
            if fmt == "pdf" or mimetype == "application/pdf" or ".pdf" in url.lower():
                out.append(resource)
        return out

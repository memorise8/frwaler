# -*- coding: utf-8 -*-
"""ICN HAL Science search crawler.

Uses the public HAL Solr API (api.archives-ouvertes.fr/search/) to collect
ART and COMM document types submitted with attached files from
ICN Business School (struct ID 462936).

Starting URL reference:
  https://icn.hal.science/search/index/?q=%2A&rows=30&submitType_s=file&docType_s=ART+OR+COMM

Note: the frontend is protected by Anubis bot-detection; the public Solr API
at api.archives-ouvertes.fr bypasses this and is the canonical way to access
HAL institutional portals programmatically.
"""
...
_STRUCT_ID = 462936  # ICN Business School
_FIELDS = (
    "docid,halId_s,title_s,abstract_s,authFullName_s,"
    "producedDate_s,submittedDate_s,publicationDate_tdate,docType_s,"
    "keyword_s,fileMain_s,files_s,doi_s,journalTitle_s,publisher_s,"
    "instStructName_s,labStructName_s,domain_s,language_s,"
    "volume_s,issue_s,page_s,serie_s,label_s,openAccess_bool"
)
_ROWS = 30
...
class IcnHalScienceSearchCrawler(BaseCrawler):
    site_id = "icn-hal-science-search"
    site_name = "Custom: icn-hal-science-search"
    base_url = "https://icn.hal.science"
    ...
    def _parse_date(raw: str) -> str:
        ...
        if not raw:
            return ""
        # Full ISO datetime: 2023-06-14T15:20:11Z or 2023-06-14 15:20:11
    ...
    def _first_str(val) -> str:
        """Return first element if list, else str of val."""
        ...
        for page in range(_MAX_PAGES):
            ...
            for doc in docs:
                ...
                try:
                    ...
                    pub_date_tdate = self._parse_date(str(doc.get("publicationDate_tdate") or ""))
                    submitted_raw = str(doc.get("submittedDate_s") or "")
                    posted_date = self._parse_date(submitted_raw)
                    published_date = produced_date or pub_date_tdate

                    # Document type / category
                    doc_type = str(doc.get("docType_s") or "")

                    # Journal and volume info
                    journal = str(doc.get("journalTitle_s") or "")
                    volume = self._first_str(doc.get("volume_s"))
                    issue = self._first_str(doc.get("issue_s"))
                    pages = str(doc.get("page_s") or "")
                    serie = self._first_str(doc.get("serie_s"))
                    label = str(doc.get("label_s") or "")
                    ...
                    # PDF URL: prefer files_s (direct PDF link), fallback to fileMain_s
                    files_s = doc.get("files_s") or []
                    if isinstance(files_s, list) and files_s:
                        pdf_url = str(files_s[0])
                    elif files_s:
                        pdf_url = str(files_s)
                    else:
                    ...
                    open_access = bool(doc.get("openAccess_bool", False))

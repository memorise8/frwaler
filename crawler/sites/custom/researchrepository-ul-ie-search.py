# -*- coding: utf-8 -*-
"""Crawler for the University of Limerick Research Repository — Article search."""
...
class ResearchrepositoryUlIeSearchCrawler(BaseCrawler):
    site_id = "researchrepository-ul-ie-search"
    site_name = "Custom: researchrepository-ul-ie-search"
    base_url = "https://researchrepository.ul.ie"

    API_BASE = "https://researchrepository.ul.ie/server/api"
    SEARCH_FILTERS = "f.itemtype=Article,equals"
    PAGE_SIZE = 20
    ...
    def _parse_item(self, uuid, indexable, full_data, detail_url):
        ...
        keywords = self._all_meta(meta, "dc.subject", "dc.subject.sdg")
        keywords = self._dedupe([self._one_line(k) for k in keywords if self._one_line(k)])

        category = self._first_meta(meta, "dc.type") or "Article"

        raw_issued = self._first_meta(meta, "dc.date.issued")
        raw_firstonline = self._first_meta(meta, "dc.date.firstonline")
        published_date = self._normalize_date(raw_issued) or self._normalize_date(raw_firstonline)
        ...
        # Journal / citation parsing — dc.identifier.citation typically looks
        # like "Journal Name, Year, Volume(Issue), pp. start-end".
        citation = self._first_meta(meta, "dc.identifier.citation")
        journal, volume, issue = self._parse_citation(citation)
        ...
        department_values = self._all_meta(meta, "dc.relation.department")
        ...
        publisher = self._first_meta(meta, "dc.publisher") or "University of Limerick"
    ...
    def _parse_citation(citation):
        """Best-effort split of a DC citation string into (journal, volume, issue).

        Typical shape: "Journal Name, Year, Volume(Issue), pp. start-end" or
        "Journal Name, Year, (Issue), pp. start-end".
        """
        if not citation:
            return None, None, None
        journal = citation.split(",")[0].strip() or None
        volume = None
        issue = None
        match = re.search(r"(\d+)\s*\((\w+)\)", citation)
        if match:
    ...
    def _extract_doi(meta):
        for key in ("dc.identifier.doi", "dc.identifier.publisherdoi"):

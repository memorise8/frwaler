# -*- coding: utf-8 -*-
"""Crawler for KIHASA research reports (전체보고서).

List:   https://www.kihasa.re.kr/publish/report/all/list?page=N
Detail: https://www.kihasa.re.kr/publish/report/view?seq=XXXXX
...
page. List items carry seq/title/writer; full contents (TOC) is only on the
detail page.
"""
...
class KihasaReKrPublishCrawler(BaseCrawler):
    site_id = "kihasa-re-kr-publish"
    site_name = "Custom: kihasa-re-kr-publish"
    base_url = "https://www.kihasa.re.kr"

    _LIST_URL = "https://www.kihasa.re.kr/publish/report/all/list"
    _DETAIL_BASE = "https://www.kihasa.re.kr/publish/report/view"
    ...
    def _extract_js_string(self, text: str, key: str) -> str:
        ...
        Handles both `key:"value"` and `"key":"value"` forms. Scans
        character-by-character to avoid regex backtracking on large strings.
        """
    ...
    def _build_paper(self, seq: int, detail_raw: str, url: str) -> dict[str, Any]:
        ...
        if not title:
            title = f"KIHASA 보고서 {seq}"

        # Author
        writer = self._extract_js_string(nuxt, "writer")
        authors = json.dumps([writer] if writer else [], ensure_ascii=False)
        ...
        # Report number (e.g. "용역보고서 2026-006")
        rno = self._extract_js_string(nuxt, "rno")

        # Category from taxonomyInfo.level1Name
        category = ""
        tm = re.search(r'level1Name:"([^"]*)"', nuxt)
        if tm:
            category = tm.group(1)

        # Keywords from taxonomyInfo.level2Name entries
        keywords_list: list[str] = []
        for km in re.finditer(r'level2Name:"([^"]*)"', nuxt):
            kw = km.group(1)
            if kw and kw not in keywords_list:
                keywords_list.append(kw)

        # Abstract — decode the contents field (HTML table of contents) and strip tags
        abstract = ""
        ...
        # Fallback: scrape the rendered detail HTML for main content blocks
        if len(abstract) < 100 and detail_raw:
            try:
                dsoup = self._make_soup(detail_raw, context=f"seq={seq} detail-html")
                for sel in (".view-content", ".board-view", ".se-contents",
                            ".content_box", "main", "article"):
        ...
        # PDF URL from first fileSeq in the files array
        pdf_url = ""
        ...
        return {
            ...
            "metadata": json.dumps({"seq": seq, "rno": rno}, ensure_ascii=False),
        }

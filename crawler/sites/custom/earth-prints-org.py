# -*- coding: utf-8 -*-
"""Earth-Prints crawler — INGV open archive for earth sciences (DSpace 7.6 REST).

The public HTML UI (``/search?configuration=researchoutputs&...``) is the
Angular DSpace 7 app (probe: 202/JS-rendered), so this crawler talks to the
DSpace 7 REST API directly:

* list:   ``/server/api/discover/search/objects?configuration=researchoutputs``
          — the ``researchoutputs`` discovery view returns research-output
          items. ``f.has_content_in_original_bundle=true`` restricts to items
          that actually carry a downloadable file (~16.9k items).
...
class EarthPrintsOrgCrawler(BaseCrawler):
    site_id = "earth-prints-org"
    site_name = "Custom: earth-prints-org"
    base_url = "https://www.earth-prints.org"

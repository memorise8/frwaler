# -*- coding: utf-8 -*-
"""Crawler for Lenus (the Irish Health Repository) — browse-by-title listing.

Data source: the site's DSpace 7.6 REST API at ``https://www.lenus.ie/server/api``.
The public UI page (``https://www.lenus.ie/browse/title``) is a JS-rendered
Angular SPA backed by this API, so we talk to the API directly (DSpace 7+
``discover/search/objects`` endpoint, sorted by ``dc.title,ASC`` to match the
"browse by title" ordering) instead of scraping rendered HTML.
"""
...
class LenusIeBrowseCrawler(BaseCrawler):
    site_id = "lenus-ie-browse"
    site_name = "Custom: lenus-ie-browse"
    base_url = "https://www.lenus.ie"

    API_BASE = "https://www.lenus.ie/server/api"
    PAGE_SIZE = 20
    ...
    def _parse_item(self, uuid, indexable, full_data, detail_url):
        ...
        abstract_values = self._all_meta(meta, "dc.description.abstract", "dc.description")
        abstract = "\n\n".join(abstract_values)
        abstract = self._clean_text(abstract)

        authors = self._all_meta(meta, "dc.contributor.author", "dc.creator")
        authors = self._dedupe(
            [self._format_author_name(self._one_line(a).strip(" ;")) for a in authors if self._one_line(a).strip(" ;")]
        )

        keywords = self._all_meta(meta, "dc.subject", "dc.subject.mesh", "dc.subject.other")
        ...
        raw_issued = self._first_meta(meta, "dc.date.issued") or self._first_meta(meta, "dc.date")
        ...
        contributor_department = self._first_meta(meta, "dc.contributor.department")
        department_parts = [p for p in (contributor_department, community_name, collection_name) if p]
        department = "; ".join(self._dedupe(department_parts)) if department_parts else None

        publisher = self._first_meta(meta, "dc.publisher") or None

        journal = self._first_meta(meta, "dc.identifier.journal") or self._first_meta(meta, "dc.source.journaltitle") or None

        doi = self._first_meta(meta, "dc.identifier.doi") or None
        series = self._first_meta(meta, "dc.relation.ispartofseries") or None
        volume = self._first_meta(meta, "dc.source.volume") or None
        issue = self._first_meta(meta, "dc.source.issue") or None
        ...
        metadata_raw = {
            ...
            "citation": self._first_meta(meta, "dc.identifier.citation") or None,
            "pmid": self._first_meta(meta, "dc.identifier.pmid") or None,
            "pmcid": self._first_meta(meta, "dc.identifier.pmcid") or None,
            "issn": self._first_meta(meta, "dc.identifier.issn") or None,
            "eissn": self._first_meta(meta, "dc.identifier.eissn") or None,

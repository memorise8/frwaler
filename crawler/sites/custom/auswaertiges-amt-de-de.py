# -*- coding: utf-8 -*-
"""Crawler for Auswärtiges Amt (German Federal Foreign Office) newsroom archive."""
...
class AuswaertigesAmtDeDeCrawler(BaseCrawler):
    site_id = "auswaertiges-amt-de-de"
    site_name = "Custom: auswaertiges-amt-de-de"
    base_url = "https://www.auswaertiges-amt.de"

    _LIST_URL = (
        "https://www.auswaertiges-amt.de"
        "/ajax/json-filterlist/de/newsroom/presse/newsroom-archiv/609192-609192"
    )
    _PAGE_SIZE = 20
    _MAX_PAGES = 200
    _MAX_RUNTIME_SECS = 25 * 60
    _MIN_ABSTRACT_CHARS = 100  # verification test requires >= 100
    _BACKOFF = (1, 3, 9)
    _CURL_TIMEOUT = 60

    def crawl(self, limit=None):
        """Crawl the Auswärtiges Amt newsroom archive."""
        saved = 0
            ...
            for item in items:
                ...
                try:
                    ...
                    detail_raw = self._curl_get(
                        ...
                        referer=(
                            "https://www.auswaertiges-amt.de"
                            "/de/newsroom/presse/newsroom-archiv"
                        ),
                    )
                    if not detail_raw:
    ...
    def _parse_detail(self, raw, list_item):
        ...
        # Fallback: any h1 that isn't the a11y screen-reader homepage title
        if not title or "Willkommen auf den Seiten" in title:
            for h in main.find_all("h1"):
                t = self._clean_text(h.get_text(" ", strip=True))
                if t and "Willkommen auf den Seiten" not in t:
                    title = t
                    break
        if not title:
        ...
        if intro_p:
            intro = self._clean_text(intro_p.get_text(" ", strip=True))

        # --- Article body: paragraphs with class rte__paragraph ---
        body_texts = []
        ...
        # Also pick up h4 headings inside article (e.g. interview Frage/Antwort headers)
        # so the abstract is self-contained
        if not body_texts:
            # Fallback: any paragraph in the first u-grid-row that has paragraphs
            for div in main.find_all("div", class_="u-grid-row"):
                ...
                for p in paras:
                    ...
                    if (
                        t
                        and t not in ("Seite drucken", "Seite teilen", "Zurück")
                        and "Seite drucken" not in t
                        and "Seite teilen" not in t
                        and "Zurück zu:" not in t
                        and "Sie befinden sich hier:" not in t
                    ):
                        body_texts.append(t)
                if body_texts:
                    break
        ...
        # --- Keywords from Schlagworte section ---
        keywords = ""
        for sec in main.find_all("section"):
            if "Schlagworte" in sec.get_text(" ", strip=True):
                kw_links = [
        ...
        # --- PDF link (rare for press releases) ---
        pdf_url = ""

# -*- coding: utf-8 -*-
"""Crawler for ENVT records in HAL Science search."""
...
class EnvtHalScienceSearchCrawler(BaseCrawler):
    site_id = "envt-hal-science-search"
    site_name = "Custom: envt-hal-science-search"
    base_url = "https://envt.hal.science"

    _START_URL = (
        "https://envt.hal.science/search/index/?q=%2A&rows=30"
        "&submitType_s=file&docType_s=ART+OR+THESE+OR+MEM+OR+COMM"
    )
    _SEARCH_API = "https://api.archives-ouvertes.fr/search/envt/"
    _PAGE_SIZE = 30
    ...
    def _fetch_list(self, start):
        params = [
            ...
            ("fq", "docType_s:(ART OR THESE OR MEM OR COMM)"),
    ...
    def _fetch_detail(self, item):
        ...
        data = self._fetch_json(self._api_url(params), label=f"detail {docid or hal_id}")
    ...
    def _paper_from_doc(self, doc):
        ...
        metadata = {
            ...
            "authority_institution": self._first_value(doc, "authorityInstitution_s"),

# -*- coding: utf-8 -*-
"""Crawler for ETH Zurich Research Collection — open-access journal articles search.

Starting URL:
  https://www.research-collection.ethz.ch/search?spc.page=1
    &f.ethzAvailability=Open%20access,equals&f.itemtype=Journal%20Article,equals

Discovery notes:
  - DSpace 7 backend. The public search UI is backed by the REST API at
    ``/server/api/discover/search/objects`` (same filter params as the UI's
    query string, just without the ``spc.`` prefix).
  - Item detail: ``/server/api/core/items/{uuid}`` with
    ``embed=owningCollection/parentCommunity,bundles/bitstreams`` pulls
    collection/community (department context) and bitstreams (PDF) in one
    request.
  - Canonical detail page URL uses the DSpace handle:
    ``https://www.research-collection.ethz.ch/handle/{handle}``.
"""
...
class ResearchCollectionEthzChSearchCrawler(BaseCrawler):
        ...
        while True:
            ...
            for idx, obj in enumerate(objects, start=1):
                ...
                try:
                    ...
                    detail_url = self._handle_url(indexable)
                    if not detail_url:
                        detail_url = f"{self.base_url}/items/{uuid}"
    ...
    def _list_url(self, page):
        return (
            ...
            f"?query=&dsoType=item&{self.SEARCH_FILTERS}"
    ...
    def _parse_item(self, uuid, indexable, full_data, detail_url):
        ...
        authors = self._dedupe(
            [self._format_author_name(self._one_line(a).strip(" ;")) for a in authors if self._one_line(a).strip(" ;")]
        )

        keywords = self._all_meta(meta, "dc.subject")
        keywords = self._dedupe([self._one_line(k) for k in keywords if self._one_line(k)])

        category = self._first_meta(meta, "dc.type") or "Journal Article"

        raw_issued = self._first_meta(meta, "dc.date.issued")
        raw_published = self._first_meta(meta, "dc.date.published")
        published_date = self._normalize_date(raw_published or raw_issued)
        ...
        department_parts = [p for p in (community_name, collection_name) if p]
        department = "; ".join(self._dedupe(department_parts)) if department_parts else None
        publisher = self._first_meta(meta, "dc.publisher") or "ETH Zurich"

        journal_title = self._first_meta(meta, "ethz.journal.title")
        journal_volume = self._first_meta(meta, "ethz.journal.volume")
        journal_issue = self._first_meta(meta, "ethz.journal.issue")

        doi = self._first_meta(meta, "dc.identifier.other")
        if not doi:
            doi_uri = self._first_meta(meta, "dc.identifier.doi")
            if doi_uri:
                doi = re.sub(r"^https?://(dx\.)?doi\.org/", "", doi_uri)
        ...
        metadata_raw = {
            ...
            "journal_raw": journal_title or None,
            "series": None,
            "volume": journal_volume or None,
            "issue": journal_issue or None,
            "item_uuid": uuid,
            ...
            "date_issued": raw_issued or None,
            "date_published": raw_published or None,
            "collection": collection_name or None,
            "community": community_name or None,
            "availability": self._first_meta(meta, "ethz.availability") or None,
            "publication_status": self._first_meta(meta, "ethz.publication.status") or None,
            "issn": self._first_meta(meta, "dc.identifier.issn") or None,
            "scopus_id": self._first_meta(meta, "ethz.identifier.scopus") or None,
            "wos_id": self._first_meta(meta, "ethz.identifier.wos") or None,

# -*- coding: utf-8 -*-
"""health-ni.gov.uk publications crawler (Drupal 10 HTML scrape)."""
...
class HealthNiGovUkPublicationsCrawler(BaseCrawler):
    site_id = "health-ni-gov-uk-publications"
    site_name = "Custom: health-ni-gov-uk-publications"
    base_url = "https://www.health-ni.gov.uk"

    _LIST_URL = "https://www.health-ni.gov.uk/publications"
    _MAX_PAGES = 200
    ...
    def _node_id(html: str) -> str | None:
        m = re.search(r'"currentPath":"node[^\d]+(\d+)"', html[:12000])
        if not m:
    ...
    def _parse_detail(self, raw: str, card_summary: str) -> dict:
        ...
        for a in main.find_all("a", class_="file-link"):
            ...
            # Strip the .meta span (file type / size) to get clean title
            try:
                a_soup = self._make_soup(str(a))
                if a_soup:
                    meta_span = a_soup.find(class_="meta")
                    if meta_span:
                        meta_span.decompose()
                    lt = self._clean(a_soup.get_text())
                    if lt:
        ...
        while True:
            ...
            for it in new_items:
                ...
                try:
                    ...
                    if len(abstract) < 100:
                        print(f"[{self.site_id}] Skipping (abstract {len(abstract)} chars < 100): {it['title'][:60]}")
                        continue

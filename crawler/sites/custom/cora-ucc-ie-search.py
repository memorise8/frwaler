# -*- coding: utf-8 -*-
"""Crawler for CORA (Cork Open Research Archive, University College Cork) — doctoral theses search."""
...
class CoraUccIeSearchCrawler(BaseCrawler):
    site_id = "cora-ucc-ie-search"
    site_name = "Custom: cora-ucc-ie-search"
    base_url = "https://cora.ucc.ie"

    API_BASE = "https://cora.ucc.ie/server/api"
    SEARCH_FILTERS = "f.itemtype=Doctoral%20thesis,equals"
    PAGE_SIZE = 20
    ...
    def _parse_item(self, uuid, indexable, full_data, detail_url):
        ...
        advisors = self._all_meta(meta, "dc.contributor.advisor")
        advisors = self._dedupe(
            [self._format_author_name(self._one_line(a).strip(" ;")) for a in advisors if self._one_line(a).strip(" ;")]
        )

        keywords = self._all_meta(meta, "dc.subject")
        keywords = self._dedupe([self._one_line(k) for k in keywords if self._one_line(k)])

        category = self._first_meta(meta, "dc.type") or "Doctoral thesis"
        ...
        title_alt = self._first_meta(meta, "dc.title.alternative")
        ...
        publisher = self._first_meta(meta, "dc.publisher") or "University College Cork"
        ...
        metadata_raw = {
            ...
            "title_alternative": title_alt or None,

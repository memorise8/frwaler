# -*- coding: utf-8 -*-
"""AMU HAL Science search crawler.

Uses the public HAL Solr API (api.archives-ouvertes.fr/search/) to collect
articles, conference papers, theses and memoirs submitted with attached files
from Aix-Marseille Université (AMU).

Starting URL reference:
  https://amu.hal.science/search/index/?q=%2A&rows=30&submitType_s=file&docType_s=ART+OR+COMM+OR+THESE+OR+MEM
"""
...
class AmuHalScienceSearchCrawler(BaseCrawler):
    ...
    def _fetch_page(self, start: int, rows: int) -> dict | None:
        ...
        params = {
            ...
            "fq": [
                "collCode_s:UNIV-AMU",
                "submitType_s:file",
                "docType_s:(ART OR COMM OR THESE OR MEM)",
            ],
        }
